...
def runtime_switch_setenv(args: argparse.Namespace, arm_root: Path) -> List[str]:
    if not getattr(args, "runtime_switch", False):
        return []
    return [
        "MF_ADAPTIVE_GUIDANCE_SWITCH=1",
        f"MF_ADAPTIVE_RANDOM_WINDOW={args.runtime_switch_window}",
        f"MF_ADAPTIVE_MIN_RANDOM_WINDOWS={args.runtime_switch_min_windows}",
        f"MF_ADAPTIVE_PLATEAU_WINDOWS={args.runtime_switch_plateau_windows}",
        f"MF_ADAPTIVE_INPUT_STALL_MAX_DELTA={args.runtime_switch_input_stall_max_delta}",
        f"MF_ADAPTIVE_COVERAGE_STALL_MAX_DELTA={args.runtime_switch_coverage_stall_max_delta}",
        f"MF_ADAPTIVE_ACCESS_REPEAT_RATIO={args.runtime_switch_access_repeat_ratio}",
        f"MF_ADAPTIVE_HOTSPOT_STABLE_WINDOWS={args.runtime_switch_hotspot_stable_windows}",
        f"MF_ADAPTIVE_HOTSPOT_TOP_K={args.runtime_switch_hotspot_top_k}",
        f"MF_ADAPTIVE_SWITCH_SUMMARY_OUT={arm_root / 'adaptive_guidance_switch_summary.json'}",
    ]
...
def run_guided_case(repo: Path, case: Dict[str, Any], repeat_idx: int, args: argparse.Namespace, fuzzer_bin: Path) -> Dict[str, Any]:
    ...
    add_setenv_args(cmd, runtime_switch_setenv(args, arm_root))
    add_setenv_args(cmd, args.setenv or [])
    rec = run_cmd(cmd, cwd=repo, log_path=arm_root / "runner.log", dry_run=args.dry_run)
...
def main() -> None:
    ...
    ap.add_argument("--runtime-switch", dest="runtime_switch", action="store_true", default=True)
    ap.add_argument("--no-runtime-switch", dest="runtime_switch", action="store_false")
    ap.add_argument("--runtime-switch-window", default="30s")
    ap.add_argument("--runtime-switch-min-windows", type=int, default=3)
    ap.add_argument("--runtime-switch-plateau-windows", type=int, default=2)
    ap.add_argument("--runtime-switch-input-stall-max-delta", type=int, default=0)
    ap.add_argument("--runtime-switch-coverage-stall-max-delta", type=int, default=0)
    ap.add_argument("--runtime-switch-access-repeat-ratio", type=float, default=0.90)
    ap.add_argument("--runtime-switch-hotspot-stable-windows", type=int, default=2)
    ap.add_argument("--runtime-switch-hotspot-top-k", type=int, default=3)
    ...
    plan = {
        ...
        "runtime_switch_enabled": bool(args.runtime_switch),
        "runtime_switch_window": args.runtime_switch_window,
