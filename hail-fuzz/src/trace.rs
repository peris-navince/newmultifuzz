...
impl PathTracerRef {
    ...
    pub fn clear(&self, vm: &mut icicle_vm::Vm) {
        let mmio_handle = {
            let path_tracer = vm.cpu.get_hook_mut(self.0);
            let tracer = path_tracer.data_mut::<PathTracer>().unwrap();
            tracer.blocks.clear();
            tracer.mmio_handler
        };

        let handler = vm.cpu.mem.get_io_memory_mut(mmio_handle).as_mut_any();
        if let Some(handler) = handler.downcast_mut::<FuzzwareMmioHandler<MultiStream>>() {
            if let Some(tracer) = handler
                .source
                .tracer
                .as_mut()
                .and_then(|x| x.as_mut_any().downcast_mut::<MultiStreamTracer>())
            {
                tracer.reads.clear();
            }
        }
